...
class YoLinkSW(YoLinkMQTTDevice):
    def __init__(yolink, yoAccess,  deviceInfo, callback):
        ...
        yolink.access_mode = 'Local'

        try:
            yolink.local_support = deviceInfo['serviceZone'] is not None
        except ValueError:
            yolink.local_support = False
    ...
    def setState(yolink, state):
        ...
        if 'setState'  in yolink.methodList:          
            ...
            if yolink.access_mode in ['cloud']:
                return(yolink.setDevice( data))
            elif yolink.access_mode in ['local']:
                return(yolink.setDeviceL(data))
            elif yolink.access_mode in ['hybrid']:
                if yolink.online:
                    return(yolink.setDevice( data))
                else:
                    return(yolink.setDeviceL(data))
